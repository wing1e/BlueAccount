  ...
  <view class="container">
    ...
    <view class="body">
      <view class="panel">
        <view class="total">
          <text class="amount">{{
            type === "expense" ? `-${data.amount}` : data.amount
          }}</text>
...
const data = computed(() => {
  if (title.value) {
    ...
    const { expense: lastExpense, income: lastIncome } = getTotal(
      timeChain(date, range, "last")
    );
    const amount = type.value === "expense" ? currentExpense : currentIncome;
    const QOQ =
      type.value === "expense"
        ? calcPercentage(currentExpense, lastExpense)
        : calcPercentage(currentIncome, lastIncome);
